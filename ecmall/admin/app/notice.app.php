...
class NoticeApp extends BackendApp 
{
      ...
      {   
            ...
            else 
            {
                ...
                if ((empty($_POST['content']) && trim($_POST['send_mode']) == EMAIL) || (empty($_POST['content1']) && trim($_POST['send_mode'] == MESSAGE)))
                {
                    $this->show_warning('no_content');
                    exit;
                }
                if (trim($_POST['send_mode'] == EMAIL) && empty($_POST['title']))
                {
                    $this->show_warning('title_empty');
                    exit;
                }
                $title = trim($_POST['send_mode']) == EMAIL ? trim($_POST['title']) : '';
                $content = trim($_POST['send_mode']) == MESSAGE ? trim($_POST['content1']) : trim($_POST['content']);
                $result = array();
                $count = 0;
