...
class ApplyApp extends MallbaseApp
{
    ...
    function index()
    {
        ...
        $auth_info = $model_auth->get($this->visitor->get('user_id'));
        if (empty($auth_info))
        {//开店人员必须先开通账户;
           $this->show_warning('has_no_auth_store');
           exit;
        }
        if ($auth_info['auth_state'] != 1)
        {//正在审核资料
           $this->show_warning('has_no_auth_store_0'); 
        }
        
        $bank = & m('bank');
        $bank_info = $bank->get($this->visitor->get('user_id'));
        if (empty($bank_info))
        {//开店人员必须先开通账户;
           $this->show_warning('has_no_bank_store');
        }
