...
class Buyer_orderApp extends MemberbaseApp
{
    ...
    function cancel_order()
    {
        ...
        else
        {
            ...
            if ($order_info['payment_code'] == 'account')
            {//虚拟账户支付
               // 更新支付记录状态为 为取消; 
               $model_pay = & m('pay');
               $cancel_reason = (!empty($_POST['remark'])) ? $_POST['remark'] : $_POST['cancel_reason'];
               $op_user = $this->visitor->get('user_id');
               $model_pay->updateToCancelByOrderId($val, $cancel_reason, $op_user);
    ...
    function confirm_order()
    {
        ...
        else
        {
            ...
            // **** 更新支付记录，并把钱转移到买家账户;
           $this->tranceOrderMoneyToSeller($order_info);
            /* 记录订单操作日志 */
            $order_log =& m('orderlog');
            $order_log->add(array(
                'order_id'  => $order_id,
     ...
     */
    public function tranceOrderMoneyToSeller($order_info)
   {
       //只有站内支付才操作
       if (isset($order_info['payment_code']) && $order_info['payment_code'] == 'account')
       {
           $order_id = $order_info['order_id'];
           // 更新支付记录状态  已支付 该为 完成状态; 并把钱从 from_user 的虚拟账户扣除，增加到 to_user 的实际账户中.  
            $bank = & m('bank');
            $op_user = $this->visitor->get('user_id');
            $bank->payCautionMoneyToUserByOrderId($order_id, $op_user, gmtime());
            
            if ($order_info['extension'] == 'auction')
            {//拍卖的订单，根据设置的比例扣除交易费;
                 //获取产生交易要产生的交易费;
                $m=  & m('auction');
                $auction_info = $m->getAuctionInfoByOrderId($order_id);
                $trade_money = $auction_info['trade_money'];
                $bank->deductionAuctionTaxToPlat($order_id, $trade_money, $op_user);
            }
            else
            {
                //扣除去交易费;给平台账户
                $bank->deductionTaxToPlat($order_id, $op_user);
            }
       }
   }
