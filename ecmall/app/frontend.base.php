...
class MemberbaseApp extends MallbaseApp
{
    ...
    {
        /* 只有登录的用户才可访问 */
        if (!$this->visitor->has_login && !in_array(ACT, array('login', 'register', 'check_user', 'get_register_code')))
        {
            if (!IS_AJAX)
            {
                header('Location:index.php?app=member&act=login&ret_url=' . rawurlencode($_SERVER['PHP_SELF'] . '?' . $_SERVER['QUERY_STRING']));
    ...
    function _get_member_menu()
    {
        ...
        $menu['im_buyer'] = array(
            ...
            'submenu'   => array(
                ...
                */
            	...
            	//买家的拍卖管理
            	'buyer_auction' => array(
            		'text' => Lang::get('buyer_auction'),
            		'url' => 'index.php?app=buyer_auction&act=pay_list',
            		'name' => 'buyer_auction',
            		'icon' => 'ico5',
            	),
            ),
       ...
       if($this->visitor->get('has_auth') || Conf::get('auth_allow'))
       {//认证会员才能开店 || 非认证会员开店允许开店
            ...
            if ($this->visitor->get('manage_store'))
            {
                ...
                */
                 ...
                 */
                //卖家的拍卖管理
                $menu['im_seller']['submenu']['auction'] = array(
       ...
       {
            $menu['member_auth'] = array(
                    'text' => Lang::get('member_auth'),
                    'url'  => 'index.php?app=member_auth',
                );
       }
        return $menu;
    }
