...
class GoodsApp extends StorebaseApp
{
    ...
    function index()
    {
        ...
        $sql = 'SELECT COUNT(goods_id) FROM ' . DB_PREFIX . 'auction_goods AS ag WHERE ag.status="' .EnumStatus::VALID . '" 
        	AND ag.start_time <= "' . $date_now . '" AND ag.end_time >= "' . $date_now . '" AND ag.goods_id = ' . $id;
        $db->getOne($sql);
        $this->assign('luck_by_auction', $db->getOne($sql) ? true : false);
		*/
        /* 更新浏览次数 */
    ...
    function qa()
    {
        ...
        else
        {
            ...
            if (Conf::get('captcha_status.goodsqa'))
            {
                ...
                {
                    $this->show_warning('captcha_failed', '返回上一页', 'index.php?app=goods&act=qa&id='.$id);
                    return;
                }
            }
            if (!empty($email) && !is_email($email))
            {
                $this->show_warning('email_not_correct', '返回上一页', 'index.php?app=goods&act=qa&id='.$id);
                return;
            }
    ...
    public function auction()
    {
    	...
    	$auction_goods['new_price'] = $auction_goods['curr_price'] + $auction_goods['step_price'];
    	
    	$_mod_auction = &m ('auction');
    	$auction = $_mod_auction->get_info($auction_id);
    	
    	$_mod_record = &m('auction_records');
    	$record = $_mod_record->getUserLastPay($auction_id, $goods_id, $user_id);
    	...
    	$auction_goods['last_time'] = Auction::getEndTimeStamp($auction_goods['end_time']) - time();
    	/* 当前位置 */
    	$curlocal = array(
    		array(
	    		'text' => Lang::get('auction_goods_list'), 'url' => 'index.php?app=auction&act=goods_list&id=' . $auction_id,
    		),
    		array('text' => Lang::get('goods_detail'))
    	);
    	$this->_curlocal($curlocal);
    	
    	/* 配置seo信息 */
    	$this->_config_seo($this->_get_seo_info($goods));
    	...
    	$auction_goods['new_price'] = $auction_goods['curr_price'] + $auction_goods['step_price'] * 1;
    	$arr_status = $_mod_auction_goods->getStatus($auction_goods, $user_id);
    	$auction_goods = array_merge($auction_goods, $arr_status);
		//商品保证金
    	$auction_goods['keep_money'] = ceil($auction_goods['new_price'] * $auction['keep_percent'] / 100);
    	
    	$arr_year = get_goods_year();
    	$auction_goods['year'] = $arr_year[$auction_goods['year']];
    	$arr_quality = get_goods_qualities();
    	$auction_goods['quality'] = $arr_quality[$auction_goods['quality']];
    	
    	$this->assign('auction_goods', $auction_goods);
    	$this->assign('auction', $auction);
    	$this->assign('store', $store_data);
    	$this->assign('goods', $goods);
    	$this->assign('record', $record);
    	$this->display('goods.auction.html');
    }
}
