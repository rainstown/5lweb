...
class MemberApp extends MemberbaseApp
{
    ...
    function register()
    {
        ...
        if (!IS_POST)
        {
            ...
            /* 导入jQuery的表单验证插件 */
            //$this->import_resource('jquery.plugins/jquery.validate.js,jquery.plugins/jquery.validate.add.js');
            ...
            $key = md5(md5(session_id()) + 'TEST_1001'); //防止盗发
            $this->assign('key', $key);
        ...
        else
        {
            ...
            $this->_check_reg_mob_code($_POST['phone_mob'], $_POST['check_code']);
            ...
            //保存手机信息;
            $m = &m('member');
            $m->edit($user_id, array('phone_mob' => $phone_mob));
            
            unset($_SESSION['reg_mob']);
            unset($_SESSION['code']);
 ...
 /**
    ...
    function mobile_edit(){
        ...
        if (!IS_POST)
        {
            ...
            $this->display('member.mobile_edit.html');
        }
        else
        {
            $orig_password  = $_POST['orig_password'];
           ...
           $m =& m('member');    //连接用户系统
           $result = $m->get_info($user_id);
            if (!$result)
            {
                $this->show_warning($m->user->get_error());
                return;
            }
            else
            {
               $email = $result['email']; 
               $ms =& ms();    //连接用户系统 -- 借助修改邮箱来验证，密码是否正确 
               $result = $ms->user->edit($this->visitor->get('user_id'), $orig_password, array(
                   'email' => $email
               ));
               if (!$result)
               ...
               $m->edit($user_id, array('phone_mob' => $mobile ));
            }
           //echo md5('123456');
           $this->show_message('edit_phone_mob_successed','index.php?app=member&act=mobile');
        }
    }
    ...
    function mobile(){
        ...
        if (!IS_POST)
        {
            ...
            $m_member = &m('member');
            $member_info = $m_member->get_info($this->visitor->get('user_id'));
            $mobile = $member_info['phone_mob'];
    ...
    function _get_member_submenu()
    {
        $submenus =  array(
            ...
            array(
                'name'  => 'edit_auth',
                'url'   => 'index.php?app=member&amp;act=auth',
            ),
        );
        if ($this->_feed_enabled)
        {
    ...
    function  auth(){
        ...
        if (!IS_POST)
        {
            ...
            /* 当前所处子菜单 */
            $this->_curmenu('edit_auth');
            $this->import_resource(array(
                'script' => 'jquery.plugins/jquery.validate.js',
            ));
            $this->_config_seo('title', Lang::get('user_center') . ' - ' . Lang::get('edit_auth'));
            
            
            $this->assign('auth', $auth_info);
            $this->display('member.auth.html');
        }
        else
        {
           

            $this->show_message('edit_auth_successed');
        }
    }
     ...
     * 验资注册的手机验证码
     */
    public function _check_reg_mob_code($mob, $code)
    {
        if(!isset($_SESSION['reg_mob']) || $mob != $_SESSION['reg_mob'])
        {//接收短信的手机和注册手机不一致
            $this->show_warning('check_reg_mob_code_error');
            exit;
        }
        if(!isset($_SESSION['code']) || $code != $_SESSION['code'])
        {//接收短信的手机和注册手机不一致
            $this->show_warning('check_code_error');
            exit;
        }
