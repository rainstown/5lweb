...
class MessageApp extends MemberbaseApp
{
    ...
    function view()
    {
        ...
        if (!IS_POST)
        {
            $message = $ms->pm->get($this->visitor->get('user_id'), $msg_id, true, $msg_id);
            if (empty($message))
            {
                $this->show_warning('no_such_message');
                return;
    ...
    function drop()
    {
        ...
        $from_id = isset($_GET['from_id']) ? trim($_GET['from_id']) : '';
        ...
        if (!$msg_ids)
        {
         ...
         if (MEMBER_TYPE == 'uc' && empty($from_id))
         {
            $ms =& msecmall();
         }
         else
    ...
    function drop_chat()
    {
        ...
        $arr_msg_ids = explode(',', $msg_ids);
        $arr_ecmall_msg = array();
        $arr_uc_msg = array();
        foreach($arr_msg_ids as $msg_id)
        {
            $arr_msg_id = explode('_', $msg_id);
            if (MEMBER_TYPE == 'uc' && empty($arr_msg_id[1]) )
            {
                $arr_ecmall_msg[] = $arr_msg_id[0];
            }
            else
            {
                $arr_uc_msg[] = $arr_msg_id[0]; 
            }
        }
        
        if (count($arr_ecmall_msg))
        {
            $msg_id = implode(',', $arr_ecmall_msg);
            $ms =& msecmall();
             if (!$ms->pm->drop($this->visitor->get('user_id'), $msg_id, $folder, true))    //删除
            {
                $this->show_warning($ms->pm->get_error());
        ...
        if (count($arr_uc_msg))
        {
            $msg_id = implode(',', $arr_uc_msg);
            $ms =& ms();
             if (!$ms->pm->drop_chat($this->visitor->get('user_id'), $msg_id, $folder, true))    //删除
            {
                $this->show_warning($ms->pm->get_error());
    ...
    function _list_message($pattern, $user_id)
    {
        ...
        if (MEMBER_TYPE == 'uc')
        {//开启了UC，公共消息读取ECMALL 的，私信读取UC的
            if ( $pattern == 'announcepm' || $pattern == 'systempm' ) 
            {//读取公共消息
                 $ms =& msecmall();
                 $pms = $ms->pm->get_list($user_id, $page, $pattern);
                 $page['item_count'] = $pms['count'];
            }
            elseif($pattern == 'newpm')
            {//新消息
                 $ms =& msecmall();
                 ...
                 $ms =& ms();
                 $page = $this->_get_page(10);
                 $pms_uc = $ms->pm->get_list($user_id, $page, $pattern);
                 $page['item_count'] = $pms['count'] + $page['item_count'];
                 $pms['data'] = array_merge($pms['data'], $pms_uc['data']);
