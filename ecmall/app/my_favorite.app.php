...
class My_favoriteApp extends MemberbaseApp
{
    ...
    function _add_collect_goods($goods_id, $keyword)
    {
        ...
        $collect_info = $model_collect->getInfo( $this->visitor->get('user_id'), $goods_id,  'goods'); 
        if ($collect_info)
        {
             /* 收藏成功 */
             $this->json_result('', 'collect_goods_exists');
             exit;
        }
    ...
    function _add_collect_store($store_id, $keyword)
    {
        ...
        $model_collect =& m('collect');
        $collect_info = $model_collect->getInfo( $this->visitor->get('user_id'), $store_id,  'store'); 
        if ($collect_info)
        {
             /* 收藏成功 */
             $this->json_result('', 'collect_store_exists');
              exit;
        }
