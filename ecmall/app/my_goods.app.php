...
class My_goodsApp extends StoreadminbaseApp
{
    ...
    function index()
    {
        ...
        foreach ($goods_list as $key => $goods)
        {
       	...
       	if (count($arr_id) > 0) 
       	{
	        ...
	        $sql = 'SELECT goods_id FROM ' . DB_PREFIX . 'auction_goods AS ag WHERE ag.status="' . EnumStatus::VALID . '" 	
	        	AND ag.start_time <= "' . $date_now . '" AND ag.end_time >= "' . $date_now . '" AND ag.goods_id IN (' . implode(',', $arr_id) . ')' ;
	        $auction_goods = $db->getAll($sql);
	        $arr_lock = array();
	        foreach ($auction_goods as $g) {
	        	$arr_lock[] = $g['goods_id'];
	        }
	        
	        foreach ($goods_list as $key => $goods)
	        {
	        	$goods_list[$key]['lock'] = in_array($goods['goods_id'], $arr_lock);
	        }
       	}
    ...
    function edit()
    {
        ...
        $lock = $db->getOne($sql) > 0 ? true : false;
        $this->assign('lock_by_auction', $lock);
        
        if ($lock) {
        	$this->show_warning('goods_is_lock_by_auction');
        	return;
        }
        ...
        if (!IS_POST)
        {
            ...
            $goods_years = array();
            $goods_years[0] = Lang::get('please_select');
            foreach (get_goods_year() as $idx => $year) 
            {
            	$goods_years[$idx] = $year;
            }
           	$this->assign('goods_years', $goods_years);
           	
           	$goods_qualities = array();
           	$goods_qualities[0] = Lang::get('please_select');
           	foreach (get_goods_qualities() as $idx => $q)
           	{
           		$goods_qualities[$idx] = $q;
           	}
           	
           	$this->assign('goods_qualities', $goods_qualities);
   ...
   function ajax_col()
   {
    ...
    function _check_post_data($data, $id = 0)
    {
        ...
        if (empty($data['goods']['size']))
        {
        	$this->_error('fill_size');
        	return false;
        }
        
        if (empty($data['goods']['weight'])) {
        	$this->_error('fill_weight');
        	return false;
        }
        
        if (empty($data['goods']['quality'])) {
        	$this->_error('fill_quality');
        	return false;
        }
        
        if (empty($data['goods']['year'])) {
        	$this->_error('fill_year');
        	return false;
        }
