...
class OrderApp extends ShoppingbaseApp
{
    ...
    function index()
    {
        ...
        if($goods_info['otype'] != 'auction') //团购不检查库存
        {
            /*  检查库存 */
        ...
        else
        {
            ...
            /* 减去商品库存 */
            if($goods_info['otype'] != 'auction') //团购不操作库存
            {
    ...
    function _get_goods_info()
    {
        ...
        switch ($_GET['goods'])
        {
            case "auction":// 拍卖
                 ...
                 * 拍卖的商品
                 */
                $records_id = isset($_GET['records_id']) ? intval($_GET['records_id']) : 0;
                $user_id  = $this->visitor->get('user_id');
                if (!$records_id || !$user_id)
                {
                    return false;
                }
                /* 获取团购记录详细信息 */
                $model_auction =& m('auction_records');
                $auction_info = $model_auction->getRecords($records_id);
                ...
                /**
                 * 不是拍到的产品，或者已生成过订单了的操作失败;
                 */
                if ($auction_info['order_id'] > '0' 
                    || $auction_info['pay_status'] > '0'
                    || $auction_info['records_status'] != 'V')
                {
                    return false;
                }
                /* 库存信息 */
                $model_goodsspec = &m('auction_goods');
                $goodsspec = $model_goodsspec->getInfo($auction_info['goods_id'],$auction_info['auction_id']);
                if (empty($goodsspec))
                {
                    return false;
                }
                /* 获取商品信息 */
                $spec_quantity = 1;
                $spec_price    = $goodsspec['curr_price'];
                $amount = 0;
                $groupbuy_items = array();
                $goods_image = empty($auction_info['default_image']) ? Conf::get('default_goods_image') : $groupbuy_info['default_image'];
                
                $the_price = $spec_price;
                $subtotal =  $the_price;
                $spec_id = 1;// 
                $spec_info = '';//拍卖留言
                $groupbuy_items[] = array(
                    'goods_id'  => $goodsspec['goods_id'],
                    'goods_name'  => $goodsspec['goods_name'],
                    'spec_id'  => $spec_id,
                ...
                $return['quantity']     =   $spec_quantity;
                $return['amount']       =   $amount;
                $return['store_id']     =   $auction_info['store_id'];
                $return['store_name']   =   $auction_info['store_name'];
                
                $return['type']         =   'material';
                $return['otype']        =   'auction';
    ...
    function _clear_goods($order_id)
    {
        switch ($_GET['goods'])
        {
            case 'auction':
                /* 拍卖产品的商品 */
                $model_auction_goods =& m('auction_records');
                //更新为 等待付款，和绑定订单ID
                $model_auction_goods->edit($_GET['records_id'], array(
                    'order_id'  => $order_id, 'pay_status' => ORDER_PENDING));
