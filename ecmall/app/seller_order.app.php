    ...
    function cancel_order()
    {
        ...
        else
        {
            ...
            foreach ($ids as $val)
            {
                ...
                /* 如果是虚拟账户支付，返回虚拟账户金额*/
                if ($order_info[$val]['payment_code'] == 'account')
                {
                    $buyer_id = $order_info[$val]['buyer_id'];
                    $order_amount = $order_info[$val]['order_amount'];
                    
                    $model_bank = & m('bank');
                    $model_bank->payCautionMoneyToAccount($buyer_id, $order_amount);
                    
                    // 更新支付记录状态为 为取消; 
                    $model_pay = & m('pay');
                    $cancel_reason = (!empty($_POST['remark'])) ? $_POST['remark'] : $_POST['cancel_reason'];
                    $op_user = $this->visitor->get('user_id');
                    $model_pay->updateToCancelByOrderId($id, $cancel_reason, $op_user);
                }
