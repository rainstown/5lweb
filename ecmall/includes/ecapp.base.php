...
/* 支付状态 */
define('PAY_ACCEPTED', 20);                  // 买家已付款.
define('PAY_FINISHED', 40);                  // 交易成功 -- 到底卖家账户
define('PAY_CANCELED', 0);                   // 交易已取消 -- 取消

/* 支付类型状态 */
define('PAY_TYPE_GOOD', 'G');               //购买商品的支付;
define('PAY_TYPE_CAUTION', 'C');                // 拍卖保证金交易; caution money
define('PAY_TYPE_TAX', 'T');                // 交易费
//define('PAY_TYPE_USER' ,'U');   //
define('FUNDS_IN', 'I');     //充值
define('FUNDS_OUT' ,'O');    //提现

define('FUNDS_BANK', 'H');     //汇款
define('FUNDS_ALIPAY' ,'A');    // 支付宝
define('FUNDS_WANGYIN' ,'W');    //网银
define('FUNDS_PLAT' ,'P');    //网银
...
EOT;
    ...
    function _get_new_message()
    {
        ...
        {
            $ms =& ms();
            $ms_count = $ms->pm->check_new($user_id);
            $ms =& msecmall();
            $ms_count = $ms->pm->check_new($user_id) + $ms_count;
        }
        else
        {
            $ms =& ms();
...
class BaseVisitor extends Object
{
    ...
    function _get_detail()
    {
        ...
        $member_info = $model_member->findAll(array(
            ...
            'join'          => 'has_store, has_auth',                 //关联查找看看是否有店铺
            'fields'        => 'email, password, real_name, logins, ugrade, portrait, store_id, state, sgrade , feed_config, auth_id, auth_state',
        ...
        if ($detail['store_id'] && $detail['state'] != STORE_APPLYING) // 排除申请中的店铺
        {
         ...
         /* 如果已经认证会员 */
        if ($detail['auth_id'] && $detail['auth_state'] != STORE_APPLYING) // 排除申请中的会员
        {
           $detail['has_auth'] = $detail['auth_id'];
        }
        return $detail;
    }
