...
class UcPassportPM extends BasePassportPM
{
     ...
     */
    function get($user_id, $pm_id, $full = false, $pl_id)
    {
        $message = outer_call('uc_pm_viewnode', array($user_id, 0, $pm_id, $pl_id));
        if (empty($message))
        {
            return array();
        }
    ...
    function get_list($user_id, $page, $folder = 'newpm')
    {
        ...
        if ($rs['count'])
        {
            foreach ($rs['data'] as $value)
            {
                ...
                $result[$value['pmid']]['content'] = $value['lastsummary'];// 修改为最新的一条； dongzhengcheng //$value['subject']; 
            }
        }
    ...
    { 
       ...
       // var_dump( array($user_id, $folder, $pm_ids));
        return;outer_call('uc_pm_delete', array($user_id, $folder, $pm_ids));
    }
    
    /**
     * 根据Pl id 删除数据
     * @author  dongzhengcheng
     * @param type $user_id
     * @param type $pl_ids
     * @param type $folder
     * @param type $relate
     */
    function drop_chat($user_id, $pl_ids)
    {
         $pl_ids = is_array($pl_ids) ? $pl_ids : array($pl_ids);
         return outer_call('uc_pm_deletechat', array($user_id, $pl_ids, 1));
    }
