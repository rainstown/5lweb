...
function uc_pm_list($uid, $page = 1, $pagesize = 10, $folder = 'inbox', $filter = 'newpm', $msglen = 0) {
	...
	//$tmp = array('uid'=>$uid, 'page'=>$page, 'pagesize'=>$pagesize, 'filter'=>$filter, 'msglen'=>$msglen);
        //var_dump($tmp);
        //echo UC_API_FUNC.'<br><br>';
        return UC_CONNECT == 'mysql' ? $return : uc_unserialize($return);
}
 ...
 * @return type
 */
function uc_pm_viewnode($uid, $type, $pmid, $plid=0) {
	$uid = intval($uid);
	$type = intval($type);
	$pmid = @is_numeric($pmid) ? $pmid : 0;
	$return = call_user_func(UC_API_FUNC, 'pm', 'viewnode', array('uid'=>$uid, 'type'=>$type, 'pmid'=>$pmid, 'plid' => $plid));
	return UC_CONNECT == 'mysql' ? $return : uc_unserialize($return);
}
