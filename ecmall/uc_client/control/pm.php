...
class pmcontrol extends base {
	...
	function onsendpm() {
		...
		if(!$fromuid) {
			// return 0; dongzhengcheng 2013-12-11 系统消息   $fromuid = 0; 
                    $this->user['uid'] = 0;
                ...
                else
                {
                    ...
                    if($this->settings['pmfloodctrl'] && $this->user['uid'] != 0) {
...
*/
	...
	function onviewnode() {
		...
		$type = $this->input('type');
		$pmid = $this->input('pmid');
                $plid = $this->input('plid');
                if ($plid)
                {//有PLID 以PLid为准 dongzhegncheng
                    $pms = $_ENV['pm']->getpmbyplid($this->user['uid'], $plid);
                }
                else
                {
                    $pms = $_ENV['pm']->getpmbypmid($this->user['uid'], $pmid);
