...
class pmmodel {
	...
	function getpmnum($uid, $type = 0, $isnew = 0) {
		...
		if(!$type) {
			...
			$newnum = $this->db->result_first("SELECT COUNT(*) FROM ".UC_DBTABLEPRE."pm_members m LEFT JOIN ".UC_DBTABLEPRE."pm_lists t ON t.plid=m.plid WHERE m.uid='$uid' $newsql AND t.pmtype='$type'");
                       // echo "SELECT COUNT(*) FROM ".UC_DBTABLEPRE."pm_members m LEFT JOIN ".UC_DBTABLEPRE."pm_lists t ON t.plid=m.plid WHERE m.uid='$uid' $newsql AND t.pmtype='$type'".'<br>';
		}
		return $newnum;
	}
	...
	function sendpm($fromuid, $fromusername, $touids, $subject, $message, $type = 0) {
		if(!$fromusername || !$touids || !$message) {
			return 0;
		}
                $touids =  is_array($touids) ? $touids :  explode(',', $touids);
	...
	function deletepmbypmid($uid, $pmid) {
		...
		$index = $this->db->fetch_first("SELECT * FROM ".UC_DBTABLEPRE."pm_indexes i LEFT JOIN ".UC_DBTABLEPRE."pm_lists t ON i.plid=t.plid WHERE i.plid='$pmid'");
		...
		if($index['authorid'] != $uid) {
			$this->db->query("UPDATE ".UC_DBTABLEPRE.$this->getposttablename($index['plid'])." SET delstatus=2 WHERE plid='$pmid' AND delstatus=0");
			$updatenum = $this->db->affected_rows();
			$this->db->query("DELETE FROM ".UC_DBTABLEPRE.$this->getposttablename($index['plid'])." WHERE plid='$pmid' AND delstatus=1");
			$deletenum = $this->db->affected_rows();
		} else {
			$this->db->query("UPDATE ".UC_DBTABLEPRE.$this->getposttablename($index['plid'])." SET delstatus=1 WHERE plid='$pmid' AND delstatus=0");
			$updatenum = $this->db->affected_rows();
			$this->db->query("DELETE FROM ".UC_DBTABLEPRE.$this->getposttablename($index['plid'])." WHERE plid='$pmid' AND delstatus=2");
			$deletenum = $this->db->affected_rows();
		}
	...
	function getpmlist($uid, $filter, $start, $ppp = 10) {
		...
		if($filter == 'newpm') {
			$addsql = 'm.isnew=1 AND ';
                    ...
                    $addsql = 't.pmtype=1 AND t.authorid > 0 AND '; //系统消息不在此范围以内
		/*性能有影响
		} elseif($filter == 'privatepm') {
		...
		*/
		...
		$query = $this->db->query("SELECT * FROM ".UC_DBTABLEPRE."pm_members m LEFT JOIN ".UC_DBTABLEPRE."pm_lists t ON t.plid=m.plid WHERE $addsql m.uid='$uid' ORDER BY m.lastdateline DESC LIMIT $start, $ppp");
                ...
                //echo "SELECT * FROM ".UC_DBTABLEPRE."pm_members m LEFT JOIN ".UC_DBTABLEPRE."pm_lists t ON t.plid=m.plid WHERE $addsql m.uid='$uid' ORDER BY m.lastdateline DESC LIMIT $start, $ppp";
